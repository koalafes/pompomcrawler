  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .loading-copy::before {{
      ...
      height: 16px;
      border-radius: 999px;
      border: 3px solid rgba(122, 75, 39, .18);
      border-top-color: var(--mint);
      animation: loading-spin .82s linear infinite;
    }}
    .skeleton {{
      position: relative;
      overflow: hidden;
      border-radius: 7px;
      background: linear-gradient(90deg, rgba(234, 215, 179, .38), rgba(255, 253, 245, .95), rgba(234, 215, 179, .38));
    ...
    .skeleton-day:nth-child(7n) {{ border-right: 0; }}
    .skeleton-line {{
      height: 12px;
      width: 100%;
    }}
    ...
    .mobile-skeleton-day:nth-child(7n) {{ border-right: 0; }}
    .mobile-skeleton-number {{
      width: 22px;
      height: 22px;
      border-radius: 999px;
    }}
...
<body class="{body_class}">
  ...
  <script>
    ...
    function renderLoading() {{
      dataSummary.innerHTML = `<span class="loading-copy">予定を読み込み中</span>`;
    ...
    function renderLoadingAgenda() {{
      agendaTitle.textContent = "これからの予定";
      agendaSubtitle.textContent = "読み込み中";
      agenda.innerHTML = `
        <div class="skeleton-agenda">
          ${{Array.from({{ length: 5 }}, () => `
            <div class="skeleton-agenda-row">
    ...
    function renderLoadingMobile() {{
      ...
      for (let index = 0; index < 35; index += 1) {{
        const cell = document.createElement("div");
        cell.className = "mobile-skeleton-day";
        cell.innerHTML = `
          <span class="skeleton mobile-skeleton-number"></span>
          <span class="skeleton mobile-skeleton-dots"></span>
        `;
        mobileGrid.append(cell);
      }}
      mobileSelected.innerHTML = `
        <div class="mobile-selected-head"><span class="loading-copy">予定を読み込み中</span></div>
        <div class="skeleton-agenda-row">
          <span class="skeleton skeleton-thumb"></span>
          <span class="skeleton-copy">
            <span class="skeleton skeleton-line"></span>
    ...
    function renderLoadError(error) {{
      dataSummary.textContent = "予定の読み込みに失敗しました";
      agendaTitle.textContent = "読み込みエラー";
    ...
    function renderCalendar(items) {{
      ...
      for (let index = 0; index < 42; index += 1) {{
        ...
        dayItems.slice(0, 2).forEach(item => list.append(eventButton(item)));
